...
@operator_view.route('/mo/<int:spid>/', methods=["GET"])
def sp_mo(spid=None):
...
@operator_view.route('/mr/<int:spid>/', methods=["GET"])
def sp_mr(spid=None):
    req = request.args if request.method == 'GET' else request.form
    if spid:
        ...
        _or = and_()
        if spnumber:
            spnumber = spnumber.strip()
            _or.append(UsrSPSync.spnumber==spnumber)
        if msg:
            msg = msg.strip()
            _or.append(UsrSPSync.extmsg==msg)
        if feeprice:
            feeprice = feeprice.strip()
            _or.append(UsrSPSync.feeprice==feeprice)
...
@operator_view.route('/ivr/<int:spid>/', methods=["GET"])
